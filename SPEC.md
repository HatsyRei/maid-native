...
- [x] **Markdown re-parse on scroll — FIXED 2026-07-28.** `MarkdownText` (`ui/markdown/Markdown.kt`) re-parsed the **entire** message (AST build + reference-link lookup) on every recomposition. Because `LazyColumn` disposes off-screen items, scrolling a long chat re-parsed each bubble every time it re-entered the viewport — idle CPU/battery burn. Resolved with a module-level access-ordered LRU (`MarkdownParseCache`, 480 entries) keyed by content string → parsed `State.Success`, rendered via the m3 `Markdown(state = …)` overload. Styling (typography/padding/annotator) is still applied fresh at render, so appearance is unchanged; parsing is delegated to the library (via `rememberMarkdownState(immediate = true)` on a miss) so reference-link resolution is preserved. The actively streaming bubble passes `cache = false` (its content changes every token) to avoid flooding the cache with transient partials. The cache is a process-lifetime object, so it is cleared on conversation switch (`clearMarkdownParseCache()` from `ChatScreen`'s `LaunchedEffect(state.root)`) to release the previous chat's retained ASTs. Per-token re-parse of the *streaming* reply remains intentional (see whole-map fix above); coalescing token updates is still the lever there.
