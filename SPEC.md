...
- **Simultaneous multi-finger long-press opens multiple menus — WON'T FIX (analysed 2026-07-30, very low priority).** Long-pressing two distinct long-pressable items (two message bubbles, two drawer pills) with two fingers at once opens *both* context menus. Both menus then behave correctly. Root cause is three layers each behaving as designed, with no coordinating authority:
  ...
  3. **Menu state is composable-local and popups are separate windows.** `menuOpen` is a per-`MessageItem` / per-`DrawerChatItem` `remember`, so nothing enforces mutual exclusion. `TapContextMenu` uses a raw `Popup`, which dismisses on outside touch via `FLAG_WATCH_OUTSIDE_TOUCH` → `ACTION_OUTSIDE`; that only fires for a `DOWN` arriving *after* the popup window is attached. Both fingers went down ~500 ms *before* either popup existed, and Android never re-routes an in-flight gesture to a window added mid-gesture — so neither popup ever sees an outside touch.

  Not a Compose deficiency: the classic View system does the same (`ViewGroup` splits pointers across children by default since API 11, each arming its own `CheckForLongPress`). "Two fingers long-pressing two things" has no more obviously-correct answer than "show both". If it ever graduates from curiosity to annoyance, the cheapest fix is to **hoist the state** — a single `openMenuId: String?` in `ChatViewModel` (or a `CompositionLocal` menu controller) so a second long-press replaces the first; this would also give a natural hook for closing menus on scroll or navigation. Blunter alternatives: disable pointer splitting on the container (breaks legitimate multi-touch in that subtree), or add a shared long-press arbiter via `CompositionLocal` (most faithful, most code).
